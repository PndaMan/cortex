...
public class NotificationHandler: NSObject, NotificationHandlerProtocol {
  ...
  func toActiveNotification(_ request: UNNotificationRequest) -> ActiveNotification {
    // CORTEX PATCH: notificationsMap only holds notifications created in THIS
    // process. Tapping a notification delivered by a previous app session (cold
    // start, or a scheduled alert firing after a relaunch) used to force-unwrap
    // nil here and crash the app before the webview even appeared. Fall back to
    // the data UNNotificationRequest itself carries.
